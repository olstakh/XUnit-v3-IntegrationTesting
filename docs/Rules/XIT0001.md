...
or remove class-level test case orderer and delegate it to assembly-level one. Another option is to inherit from `DependencyAwareTestCaseOrderer` which allows overriding the order of tests that don't affect declared topological order (i.e. tests on the same level of dependency)
