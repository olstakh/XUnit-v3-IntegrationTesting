...
/// <summary>
/// Base attribute for test methods that participate in dependency-aware ordering and skipping.
/// Subclassed by <see cref="FactDependsOnAttribute"/> and <see cref="TheoryDependsOnAttribute"/>.
/// </summary>
[AttributeUsage(AttributeTargets.Method, AllowMultiple = false)]
public abstract class DependsOnAttributeBase(
...
internal class SkipValidator
{
    ...
    private static bool ShouldSkipTestOriginalSetup(string? originalSkipWhen, string? originalSkipUnless, string? originalSkip, Type? originalSkipType, IXunitTestCase testCase)
    {
        var originalSkipWhenFieldValue = originalSkipWhen;
        var originalSkipUnlessFieldValue = originalSkipUnless;
        var originalSkipReasonFieldValue = originalSkip;
        var originalSkipTypeFieldValue = originalSkipType;
    ...
    {
        // Get dependencyOn attribute (base type covers both FactDependsOn and TheoryDependsOn)
        var dependsOn = currentTestCase.TestMethod.Method.GetCustomAttribute<DependsOnAttributeBase>(true);

        if (dependsOn == null)
            return false;

        if (ShouldSkipTestOriginalSetup(dependsOn.OriginalSkipWhen, dependsOn.OriginalSkipUnless, dependsOn.OriginalSkip, dependsOn.OriginalSkipType, currentTestCase))
            return true;
